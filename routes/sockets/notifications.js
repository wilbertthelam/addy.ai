...
function updateVoteNotif(io, data) {
	...
	'(SELECT count(*) FROM addy_ai_football.matchups m, addy_ai_football.votes v WHERE m.league_id = ? AND m.week = ? AND m.year = ? ' +
	'AND v.matchup_id = m.matchup_id and v.user_id = ? GROUP BY league_id, week, year) = ' +
	'(SELECT count(*) FROM addy_ai_football.matchups WHERE league_id = ? AND week = ? AND year = ?) as filled_out;';

	connection.query(statement, [data.leagueId, week, year, data.userId, data.leagueId, week, year], function (err, results) {
		if (!err) {
			...
			io.emit('voteNotif', { leagueId: data.leagueId, result: newResult });
		}
	});
}
